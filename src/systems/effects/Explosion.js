...
export class ExplosionSystem {
  ...
  spawnAsteroidImpact(position) {
    this.spawnHitFlash(position, 2.5);
    this.spawnSparks(position, 60, 4.5);
  ...
  // Hiệu ứng nổ Boss đặc biệt (quy mô lớn)
  ...
  update(delta) {
    if (this.warningActive) {
      ...
      const blink = Math.sin(this.warningTime * 15) * 0.3 + 0.3;
      if (this.warningDOM) {
    ...
    for (let i = 0; i < this.engineHeatEffects.length; i++) {
      ...
      heatFx.mesh.material.opacity = heatFx.baseOpacity + Math.sin(heatFx.time * 15) * 0.03;
    }
  }
}
