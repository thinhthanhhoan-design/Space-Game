...
export class Player { // Định nghĩa lớp Player quản lý phi thuyền người chơi
    ...
    constructor(scene) {
        ...
        loader.load(
            ...
            (glb) => { // Hàm callback được gọi khi tải thành công
                ...
                this.mesh.scale.set(0.01, 0.01, 0.012); // Giảm kích thước mô hình xuống 100 lần để phù hợp với tỉ lệ thế giới game
    ...
    // Kích hoạt/Vô hiệu hóa giáp bảo vệ
    setShield(active) {
        if (!this.mesh) return;
    ...
    update(delta, enemies = []) {
        ...
        if (!this.velocity) this.velocity = { x: 0, y: 0 };
        
        const acceleration = this.speed * 0.25; // Tăng độ nhạy tăng tốc (cũ: 0.15)
        const friction = 0.9; // Giảm ma sát để đạt tốc độ tối đa cao hơn (cũ: 0.85)
        
        // Tính toán hướng di chuyển mục tiêu
        let targetDx = 0;
        let targetDy = 0;
        if (this.keys.KeyA) targetDx -= this.speed;
        if (this.keys.KeyD) targetDx += this.speed;
        if (this.keys.KeyW) targetDy += this.speed;
        if (this.keys.KeyS) targetDy -= this.speed;
