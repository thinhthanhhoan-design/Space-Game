...
export class CinematicEffects {
    constructor(scene, camera, musicSystem) {
        this.scene = scene;
        this.camera = camera;
    ...
    stopAll() {
        if (this.filmTimeline) {
            this.filmTimeline.kill();
            this.filmTimeline = null;
        }
    ...
    initDOM() {
        ...
        Object.assign(this.textDiv.style, {
            ...
            color: "white", fontSize: "24px", fontFamily: "'Orbitron', sans-serif",
            textAlign: "center",
            textShadow: "0 0 20px rgba(0, 255, 255, 0.8), 0 0 5px black",
    ...
    showText(text, duration = 3) {
        if (this.textTimeline) this.textTimeline.kill();
        this.textDiv.innerText = "";
        const tl = gsap.timeline();
        this.textTimeline = tl;
    ...
    startTunnelEffect(targetMesh, onComplete) {
        ...
        this.warpCore.scale.set(30, 30, 1);
        ...
        for (let i = 0; i < 30; i++) {
            ...
            const cloudMat = new THREE.SpriteMaterial({
                map: glowTex,
                color: isBlue ? 0x0088ff : 0x00ccaa,
                transparent: true,
        ...
        const streakColors = [0xffffff, 0xffffff, 0xffffff, 0xffffff, 0x00ffff, 0x00aaff];
