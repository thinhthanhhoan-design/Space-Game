...
export class Background { // Khai báo lớp Background quản lý nền vũ trụ và các hiệu ứng môi trường
    ...
    update(delta = 0.016) { // Phương thức cập nhật chuyển động cho nền mỗi khung hình
        const moveStars = (points, speed, rotX, rotY) => {
            if (!points) return;
            // Xoay nhẹ
            points.rotation.y += rotY;
            points.rotation.x -= rotX;
            
            // Bay về phía camera (trục z dương)
            const positions = points.geometry.attributes.position.array;
            for (let i = 0; i < positions.length; i += 3) {
                positions[i + 2] += speed * delta * 60; 
                if (positions[i + 2] > 200) { // Nếu bay qua khỏi người chơi
                    positions[i + 2] = -600 - Math.random() * 100; // Khởi tạo lại ở tít phía xa
                }
            }
            points.geometry.attributes.position.needsUpdate = true;
        };

        moveStars(this.starsGlow, 1.0, 0.00004, 0.00008);
        moveStars(this.whiteStars, 1.5, 0.00005, 0.00010);
        moveStars(this.crossStars, 2.5, 0.00003, 0.00006);
    } // Kết thúc update
