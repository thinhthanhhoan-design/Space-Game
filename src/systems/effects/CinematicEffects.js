...
export class CinematicEffects { // Lớp quản lý các hiệu ứng điện ảnh và chuyển cảnh
    constructor(scene, camera, musicSystem) { // Khởi tạo với scene và camera hiện tại
        ...
        this.filmTimeline = null; // Lưu trữ timeline của các cảnh quay chính
        this.textTimeline = null; // Lưu trữ timeline riêng cho hiệu ứng chữ phụ đề
        this.activeTimeline = null; // Giữ lại để tương thích ngược nếu cần

        this.initDOM(); // Gắn các thẻ HTML ảo dọn đường sẵn vào body
    }

    stopAll() {
        // 1. Dừng mọi GSAP đang chạy (bao gồm cả film và text)
