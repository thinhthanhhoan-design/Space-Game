...
export class GameManager { // Khai báo lớp GameManager - "Bộ não" tổng chỉ huy của trò chơi
    constructor() { // Hàm khởi tạo thực thể GameManager
        ...
        this.player.weapon.setMusicSystem(this.musicSystem);

        // Bắt sự kiện click để phát nhạc (Tránh bị trình duyệt chặn Autoplay)
        window.addEventListener('click', () => {
            this.musicSystem.unlockAudio();
        }, { once: true });
        // --- DEBUG SHORTCUTS ---
    ...
    handleSkip() {
        ...
        if (!this.stateManager.isGameStarted) {
            ...
            this.musicSystem.stopIntroSignalNoiseLoop();
            this.musicSystem.switchToBattleMusic();
            return;
        }
            ...
            const checkModel = setInterval(() => { // Thiết lập bộ đếm thời gian hỏi định kỳ 100ms
                if (this.player.mesh) { // Kiểm tra nếu mô hình 3D chiếc máy bay đã được tải xong từ server
                    ...
                    this.intro.startTransition(logoPoints, this.player.mesh, this.sceneController.camera, () => {
                        this.musicSystem.startIntroSignalNoiseLoop();
                        ...
                        setTimeout(() => {
                            ...
                            this.cinematicEffects.runShortFilm(CONFIG.CINEMATIC.SHOTS, () => {
                                ...
                                this.musicSystem.playIntroSignalNoise();

                                setTimeout(() => {
                                    // Tắt sao bay để dồn sự chú ý vào hố đen
                                    this.cinematicEffects.stopSpeedLines();
                                    this.musicSystem.playBlackHoleSuction();
        ...
        if (!this.stateManager.isGameStarted) {
            ...
            if (this.player && this.player.mesh) { // Đảm bảo mô hình tàu đã sẵn sàng
                ...
                if (dmg) {
                    ...
                    this.musicSystem.playShipDamaged();
                ...
                if (this.gamePlayState === 'WAVES') {
                    ...
                    this.combat.update(this.player, this.enemyManager.enemies, this.asteroidSystem.asteroids, this.explosionSystem, this.particleSystem, this.sceneController, this.musicSystem);
                    ...
                    if (this.boss) {
                        ...
                        this.combat.update(this.player, combatEnemies, this.asteroidSystem.asteroids, this.explosionSystem, this.particleSystem, this.sceneController, this.musicSystem);
                        ...
                        if (this.boss.isDead) {
                            ...
                            this.musicSystem.playBossFinalExplosion();
            ...
            if (this.player?.mesh) {
                this.asteroidSystem.checkBubbleCollisions(this.player.mesh, (bubbleDamage, hitPos) => {
                    this.player.takeDamage(bubbleDamage);
                    this.musicSystem.playShipDamaged();
                    if (this.explosionSystem) {
                        this.explosionSystem.spawnHitFlash(hitPos, 1.1);
                        this.explosionSystem.spawnShockwave(hitPos, 0x66ff99, 6);
                    }
                });
            }
