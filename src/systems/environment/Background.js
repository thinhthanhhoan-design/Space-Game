...
export class Background { // Khai báo lớp Background quản lý nền vũ trụ và các hiệu ứng môi trường
    ...
    init(scene, texturePath = '/textures/background.png') { // Phương thức khởi tạo các thành phần nền
        ...
        // --- HỆ THỐNG SAO BĂNG (SHOOTING STARS) ---
        this.shootingStars = [];
        for (let i = 0; i < 5; i++) { // Duy trì 5 sao băng dự phòng
            // Tăng kích thước bù đắp khoảng cách xa
            const geo = new THREE.CylinderGeometry(2.0, 0.2, 300, 4);
            geo.rotateX(Math.PI / 2); // Xoay doc theo trục Z để lookAt hoạt động chính xác
            const mat = new THREE.MeshBasicMaterial({
                color: 0xccffff, // Ánh trắng pha chút xanh dương
                transparent: true,
                opacity: 0.9,
                blending: THREE.AdditiveBlending,
                depthWrite: false,
    ...
    update(delta = 0.016, targetPos = null) { // Nhận thêm targetPos để neo hệ sao
        ...
        if (Math.random() < 0.005 && this.shootingStars) { // ~Tương đương vài giây có 1 vệt xuất hiện
            ...
            if (inactiveStar) {
                ...
                inactiveStar.mesh.material.opacity = 0.6 + Math.random() * 0.4; // Sáng ngẫu nhiên
                
                // Đặt ở vị trí đủ xa nhưng KHÔNG VƯỢT QUÁ far plane của camera (1000)
                // Camera đang ở Z=250, nên Z max nhìn thấy là -750
                const startX = (Math.random() - 0.5) * 1200;
                const startY = (Math.random() - 0.5) * 600 + 300; // Nửa trên màn hình
                inactiveStar.mesh.position.set(startX, startY, -500 - Math.random() * 200); // Rơi vào khoảng Z=-500 đến -700
                
                // Bay hướng về phía người chơi (màn hình) với tốc độ cao
                const signX = startX > 0 ? -1 : 1; // Hơi chéo vào giữa
                const signY = Math.random() > 0.5 ? -1 : 1; // Chéo lên/xuống nhẹ
                inactiveStar.dir.set(
                    signX * (0.2 + Math.random() * 0.3), 
                    signY * (0.1 + Math.random() * 0.2), 
                    1.5 + Math.random() * 1.5 // Trục Z dương lớn để lao thẳng về màn hình
                ).normalize();
                
                // Chỉnh đuôi bám sát đường bay
                const lookTarget = inactiveStar.mesh.position.clone().add(inactiveStar.dir);
                inactiveStar.mesh.lookAt(lookTarget);
                
                // Vận tốc cực đại vì bay ở vùng không gian rất xa
                inactiveStar.speed = 2500 + Math.random() * 1000; 
            }
        }
        ...
        if (this.shootingStars) {
            this.shootingStars.forEach(star => {
                if (star.active) {
                    ...
                    // Giảm mờ chậm hơn vì bay xa
                    star.mesh.material.opacity -= delta * 0.4;
