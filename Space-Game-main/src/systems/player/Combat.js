...
export class Combat {
     ...
     */
    update(player, enemies = [], asteroids = [], explosionSystem = null, particleSystem = null, sceneController = null, musicSystem = null) {
            ...
            if (enemy.mesh && !enemy.isDead && !enemy.userData?.markedForDeletion) {
                ...
                if (isCrash) {
                    ...
                    if (player.hasShield) {
                        ...
                        musicSystem?.playShipDamaged?.();
                        if (explosionSystem) explosionSystem.startWarning(0.4);
                        if (sceneController) sceneController.triggerShake(0.4, 0.2);
                    }

                    if (explosionSystem) {
                        explosionSystem.spawnShipImpact(playerPos, 4.0); 
                        explosionSystem.spawnShockwave(playerPos, 0xffffff, 32);
                        explosionSystem.spawnDebris(playerPos, 40); 
                    }
                    musicSystem?.playEnemyOrAsteroidExplode?.();
                    enemy.die();
                }
            }
        }
        ...
        for (let i = 0; i < asteroids.length; i++) {
            ...
            if (astMesh && !userData.markedForDeletion) {
                if (MathUtils.checkSphereCollisionSq(playerPos, astMesh.position, this.playerHitboxRadiusSq, this.asteroidHitboxRadiusSq)) {
                    ...
                    if (explosionSystem) {
                        explosionSystem.spawnAsteroidImpact(astMesh.position);
                        explosionSystem.spawnShockwave(astMesh.position, 0xffaa00, 15);
                        explosionSystem.spawnDebris(astMesh.position, 15);
                    }
                    musicSystem?.playEnemyOrAsteroidExplode?.();
                    ...
                    if (enemy.mesh && !enemy.isDead && !enemy.userData?.markedForDeletion) {
                        ...
                        if (isHit) {
                            ...
                            if (explosionSystem) {
                                ...
                                explosionSystem.spawnShipImpact(bulletPos, 5.5);
                                explosionSystem.spawnShockwave(bulletPos, 0x00ffff, 20);
                            }
                ...
                for (let k = 0; k < asteroids.length; k++) {
                    ...
                    if (astMesh && !userData.markedForDeletion) {
                        if (MathUtils.checkSphereCollisionSq(bulletPos, astMesh.position, this.bulletHitboxRadiusSq, this.asteroidHitboxRadiusSq)) {
                            if (explosionSystem) {
                                explosionSystem.spawnAsteroidImpact(astMesh.position);
                                explosionSystem.spawnShockwave(astMesh.position, 0xffaa00, 12);
                                explosionSystem.spawnDebris(astMesh.position, 8);
                            }
