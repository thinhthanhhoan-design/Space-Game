...
export class Background { // Khai báo lớp Background quản lý nền vũ trụ và các hiệu ứng môi trường
    constructor() { // Hàm khởi tạo của lớp Background
        ...
        this.shootingStars = null; // Mảng chứa các đối tượng sao băng
    ...
    init(scene, texturePath = '/textures/background.png') { // Phương thức khởi tạo các thành phần nền
        ...
        // --- HỆ THỐNG SAO BĂNG (SHOOTING STARS / WARP STREAKS) ---
        const createShootingStarTexture = () => {
            ...
            // Gradient tạo hiệu ứng vệt sáng lao tốc độ cao
            const gradient = ctx.createLinearGradient(0, 0, 0, 256);
            gradient.addColorStop(0.0, 'rgba(255, 255, 255, 1)');   // Đầu sáng chói trắng
            gradient.addColorStop(0.1, 'rgba(150, 255, 255, 1)');   // Chuyển sang Cyan rực rỡ
            gradient.addColorStop(0.5, 'rgba(0, 100, 255, 0.4)');   // Đuôi xanh dương mờ
            gradient.addColorStop(1.0, 'rgba(0, 0, 0, 0)');         // Tan biến hoàn toàn
        ...
        this.shootingStars = [];
        for (let i = 0; i < 5; i++) { // Duy trì 5 sao băng dự phòng
            // Thu nhỏ sao băng lại một chút cho đỡ thô, nhưng vẫn giữ độ dài để tạo cảm giác tốc độ
            const geo = new THREE.CylinderGeometry(3.5, 0.1, 400, 8);
            geo.rotateX(Math.PI / 2); // Xoay doc theo trục Z, đầu to (+Y) quay về phía trước (+Z)
            const mat = new THREE.MeshBasicMaterial({
                map: starTexture, // Sử dụng gradient phát sáng
                color: 0xffffff, 
                transparent: true,
                opacity: 0.9,
                blending: THREE.AdditiveBlending,
                ...
                fog: false // Bỏ qua màn sương mù để chiếu sáng rực qua lớp sương
            });
            const star = new THREE.Mesh(geo, mat);
            star.visible = false; // Mặc định ẩn, chỉ hiện ra khi được pick
            this.group.add(star);
    ...
    update(delta = 0.016, targetPos = null) { // Nhận thêm targetPos để neo hệ sao
        ...
        // --- CẬP NHẬT SAO BĂNG ---
        if (Math.random() < 0.005 && this.shootingStars) { // ~Tương đương vài giây có 1 vệt xuất hiện
            const inactiveStar = this.shootingStars.find(s => !s.active);
            if (inactiveStar) {
                inactiveStar.active = true;
                inactiveStar.mesh.visible = true;
                inactiveStar.mesh.material.opacity = 0.8 + Math.random() * 0.2; // Sáng rực rỡ
                
                // Đẩy sao băng dạt ra 2 bên và lên/xuống xa hơn để không bay trúng người chơi
                const signX = Math.random() > 0.5 ? 1 : -1;
                const signY = Math.random() > 0.5 ? 1 : -1;
                
                // X và Y cách tâm ít nhất 200/150 đơn vị, lan rộng đến 1000/800 đơn vị
                const startX = signX * (200 + Math.random() * 800); 
                const startY = signY * (150 + Math.random() * 650); 
                // Xuất phát từ Z cực kỳ xa (-1500 đến -2500)
                const startZ = -1500 - Math.random() * 1000;
                
                inactiveStar.mesh.position.set(startX, startY, startZ); 
                
                // ĐỂ SAO BĂNG LAO THẲNG VÀO MÀN HÌNH MỘT CÁCH HOÀN HẢO (HIỆU ỨNG WARP SPEED):
                inactiveStar.dir.set(0, 0, 1); 
                
                // SỬA LỖI XIÊN NGANG: Vì mesh nằm trong group (được di chuyển theo player), lệnh lookAt() 
                // sẽ bị sai vì lookAt nhận toạ độ World nhưng ta lại truyền toạ độ Local.
                // Do geometry đã được xoay sẵn (rotateX) để thân sao băng nằm dọc theo trục Z local,
                // ta chỉ cần reset rotation về 0 là nó sẽ chĩa thẳng tắp về phía camera.
                inactiveStar.mesh.rotation.set(0, 0, 0);
                
                // Vận tốc bay cực nhanh về phía màn hình
        ...
        if (this.shootingStars) {
            this.shootingStars.forEach(star => {
                if (star.active) {
                    // Cập nhật toạ độ dựa trên hướng và vận tốc
