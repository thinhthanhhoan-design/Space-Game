...
export class Background { // Khai báo lớp Background quản lý nền vũ trụ và các hiệu ứng môi trường
    ...
    init(scene, texturePath = '/textures/background.png') { // Phương thức khởi tạo các thành phần nền
        ...
        const createCloudTexture = () => {
            ...
            const gradient = ctx.createRadialGradient(128, 128, 0, 128, 128, 128); // Tạo gradient tròn lớn hơn cho mây
            gradient.addColorStop(0, 'rgba(120, 50, 255, 0.15)'); // Màu tím nhạt ở trung tâm với độ mờ thấp
        ...
        const cloudGeo = new THREE.PlaneGeometry(150, 150); // Tạo hình vuông mặt phẳng cạnh 150 để dán texture mây
        const cloudMaterial = new THREE.MeshBasicMaterial({ // Vật liệu cho mây tinh vân
            map: cloudTexture,
            transparent: true,
            opacity: 0.8,
            blending: THREE.AdditiveBlending, // Chế độ cộng sáng
            depthWrite: false, 
            side: THREE.DoubleSide // Hiển thị cả hai mặt của Plane
        });
        ...
        for(let i = 0; i < 80; i++) { // Sinh ra 80 đám mây ngẫu nhiên
            const cloud = new THREE.Mesh(cloudGeo, cloudMaterial); // Tạo một đám mây
            ...
            // Xoay đám mây ngẫu nhiên để trông tự nhiên hơn
    ...
    update() { // Phương thức cập nhật chuyển động cho nền mỗi khung hình
        ...
        if (this.cloudsGroup) { // Nếu có các đám mây
            ...
            this.cloudsGroup.children.forEach(cloud => { // Duyệt qua từng tấm mây con
                cloud.rotation.z += 0.00015; // Tự xoay nhẹ từng tấm mây để tạo cảm giác bồng bềnh
            });
        }
    } // Kết thúc update
